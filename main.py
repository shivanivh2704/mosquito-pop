...
pygame.mixer.music.play(-1)    # indicates infinite looping
# Variables
state = "menu"
...
def user_events():
    for event in pygame.event.get():
        if event.type == pygame.QUIT: #Exits the game when the user closes the window.
            pygame.quit()
            sys.exit()

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE: #Exits the game when the Escape key is pressed.
                pygame.quit()
                sys.exit()
